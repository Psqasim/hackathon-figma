...
const Navbar: React.FC = () => {
  ...
  const [searchNotification, setSearchNotification] = useState<string | null>(
    null
  );
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    const matchedCategory = categories.find(
      (cat) =>
        query === cat.label.toLowerCase() ||
        cat.keywords.some((keyword) => query.includes(keyword))
    );
              ...
              <div className="flex items-center gap-2 text-blue-500">
                <SignInButton mode="modal">
                  <button className="cursor-pointer">
                    <CiUser className="w-6 h-10 font-bold" />
                  </button>
                </SignInButton>
              </div>
            )}
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                >
                  <Link href="/Shop">Shop</Link>
                  <RiArrowDropDownLine
                    className={`text-xl ${isShopDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
